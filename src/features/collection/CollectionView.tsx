...
export default function CollectionView({
  ...
  // Mapear CMS Products
  const cmsItems = products.map((p: SanityProduct) => ({
  ...
  const itemsToDisplay =
    cmsItems.length > 0
      ? cmsItems
      : COLLECTION_ITEMS.filter((item) => item.type === "product");
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-[1920px] mx-auto px-6 md:px-12 pt-20 pb-8">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-2xl md:text-3xl font-bold uppercase tracking-[0.08em] text-balance">
            NEW ARRIVALS
          </h1>
          ...
          <div className="flex items-center gap-6">
            <button
              ...
              className={`flex items-center gap-2 text-[11px] uppercase tracking-[0.12em] transition-all duration-300 ease-in-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2 rounded-xl ${
                viewMode === "grid"
                  ? "opacity-100"
            ...
            <button className="flex items-center gap-2 text-[11px] uppercase tracking-[0.12em] hover:opacity-60 transition-all duration-300 ease-in-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2 rounded-xl">
      ...
      <div className="w-full px-1 mb-10">
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-y-8 gap-x-1">
          {firstBatch.map((item) => (
            ...
            >
              {/* Imagen */}
              <div className="relative aspect-3/4 w-full bg-[#F0F0F0] rounded-2xl overflow-hidden">
                {/* Imagen Principal */}
                <NextImage
                  src={item.image || ""}
                  alt={item.name || ""}
                ...
                {item.isNew && (
                  <div className="absolute top-4 left-4 text-[10px] font-medium tracking-[0.15em] text-black z-10 uppercase bg-white/80 backdrop-blur-sm px-2.5 py-1 rounded-full">
                    NEW
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col px-1">
                ...
                <div className="flex items-start justify-between w-full mb-1">
                  <h3 className="text-[13px] uppercase tracking-[0.08em] font-medium text-black pr-4 leading-tight">
                    {item.name}
                  </h3>
                ...
                <div className="flex items-center justify-between min-h-[20px]">
                  ...
                  {item.colors && item.colors.length > 0 ? (
                    <div className="flex gap-1">
                      {item.colors.map((color: string, index: number) => (
                        <div
                          key={index}
                          className="w-3 h-3 rounded-full border border-gray-200 transition-transform duration-300 hover:scale-125"
                  ...
                  {item.sizes && item.sizes.length > 0 && (
                    <div className="hidden lg:flex gap-2 text-[11px] uppercase tracking-widest text-gray-800 opacity-0 group-hover:opacity-100 transition-all duration-300 ease-in-out">
